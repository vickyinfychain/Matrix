...
import DividendPoolRecord from "../models/DividendPoolRecord.js"; // optional history
import ReentryEvent from "../models/ReentryEvent.js";
import Parameter from "../models/Parameter.js"; // 🔥 NEW: global parameters (dividend pool)
...
export async function distributeDividend() {
  // 1) Load parameter & pool
  let params = await Parameter.findOne();
  if (!params || !params.dividend || params.dividend <= 0) {
    // Nothing to distribute
    return { distributed: 0, eligibleUsers: 0 };
  }

  const pool = params.dividend;

  // 2) Fetch users who have invested something
  const users = await User.find({
    totalInvested: { $gt: 0 },
  });

  // 3) Filter eligible users (under ROI cap)
  const eligibleUsers = users.filter((u) => isUserEligibleForDividend(u));

  if (eligibleUsers.length === 0) {
    // Nobody eligible => keep pool as is
    return { distributed: 0, eligibleUsers: 0 };
  }

  // 4) Equal base share
  const baseShare = pool / eligibleUsers.length;

  let totalDistributed = 0;

  // 5) Distribute
  for (const user of eligibleUsers) {
    const remainingCap = (user.roiCap || 0) - (user.totalEarned || 0);
    if (remainingCap <= 0) continue;

    // user gets min(baseShare, remainingCap)
    const payout = Math.min(baseShare, remainingCap);

    if (payout <= 0) continue;

    // Update user fields
    user.totalEarned = (user.totalEarned || 0) + payout;
    user.dividend = (user.dividend || 0) + payout;
    await user.save();

    totalDistributed += payout;

    // Optional: record earning line
    await Earning.create({
      user: user._id,
      userId: user.userId,
      slot: null, // or some "global" slot, if you prefer
      slotNumber: 0, // mark as 0 for global dividend
      amountUSD: payout,
      type: "DIVIDEND", // distinguish from LEVEL1/2/3
      sourcePosition: null,
      uplinePosition: null,
    });
  }

  // 6) Deduct only what was used (if caps limited some users, leftover stays)
  params.dividend = 0;
  if (params.dividend < 0) params.dividend = 0; // safety
  await params.save();
  console.log({
    distributed: totalDistributed,
    eligibleUsers: eligibleUsers.length,
    remainingPool: params.dividend,
  });
  return {
    distributed: totalDistributed,
    eligibleUsers: eligibleUsers.length,
...
export async function registerUser(walletAddress, sponsorUserId) {
  ...
  user = await User.create({
    ...
    // NEW: ensure defaults exist (also in schema)
...
export async function updateMatrixCountsAndCheckCompletion(newPosition) {
  ...
  while (current) {
    ...
    const matirxCount =
      current.matrixCounts.level1 +
      current.matrixCounts.level2 +
      current.matrixCounts.level3;
...
/* ------------------------------ trigger reentry ---------------------------- */
export async function triggerReentryForPosition(
  completedPosition,
  triggeredByPosition
) {
  ...
  const cycleIndexAfter = lastPos
    ? lastPos.cycleIndex + 1
    : cycleIndexBefore + 1;
...
export async function distributeSlotPayment(
  position,
  slot,
  { isReentry = false } = {}
) {
...
export async function creditLevelIncome(
  uplinePosition,
  amount,
  fromPosition,
  type
...
export async function recordDividendContribution(amount, position, slot) {
  ...
  // 🔥 NEW: add to global dividend pool in Parameter

  let params = await Parameter.findOne();

  if (!params) {
    params = await Parameter.create({ dividend: 0 });
  }

  params.dividend = (params.dividend || 0) + amount;
...
export async function activateSlot(user, slotNumber) {
  ...
  // 🔥 Global investment tracking on User level

  user.totalInvested = (user.totalInvested || 0) + slot.priceUSD;

  user.roiCap = user.totalInvested * ROI_MULTIPLIER;

  // Optional: keep UserSlot.roiCap in sync with global cap
...
/* --------------------- Helper: dividend eligibility check ------------------ */

export function isUserEligibleForDividend(user) {
  if (!user.totalInvested || !user.roiCap) return false;

  return (user.totalEarned || 0) < user.roiCap;
}
...
export async function getUserDashboard(userId) {
  ...
  const totalEarned = earningsAgg.length > 0 ? earningsAgg[0].totalEarned : 0;
  ...
  return {
    ...
    stats: {
      totalEarned, // from Earning collection
      ...
      globalTotalInvested: user.totalInvested || totalInvested,

      globalTotalEarned: user.totalEarned || totalEarned,

      roiCap: user.roiCap || (user.totalInvested || 0) * ROI_MULTIPLIER,

      dividendTotal: user.dividend || 0,
...
export async function getSlotDetails(slotNumber) {
  ...
  const totalInvestedAgg = await UserSlot.aggregate([
  ...
  return {
    ...slot,
    activeUsers,
    totalInvested: totalInvestedAgg.length > 0 ? totalInvestedAgg[0].total : 0,
  };
}

const activateBulk = async () => {
  for (let i = 2; i < 41; i++) {
    const user = await User.findOne({ walletAddress: `a${i}` });

    await activateSlot(user, 2);
  }
  console.log("object");
};
